...
if ($mode == "prior") {
    ...
    $sql = "Select wp.* From df_site_{$table} wp Where 1 = 1 AND code = '{$code}' ";
    ...
    if ($posi == 'up') {
        ...
        if ($row = $db->row($sql)) {
            ...
            $db->query("Update df_site_{$table} Set prior=prior-1 Where prior<='{$prior}' And idx!='{$idx}' AND code = '{$code}' ");
        }
    } elseif ($posi == 'upup') {
    ...
    complete(
        "진열순서를 변경하였습니다.",
        "/Madmin/bbs/{$table}_list.php?page={$page}&{$param}"
    );
}
